...
def perfil(id_usuario):
    if int(id_usuario) == int(current_user.id):
        ...
        form_foto = FormFoto()
        fotos = Foto.query.filter_by(id_usuario=current_user.id).order_by(Foto.data_criacao.desc(),Foto.id.desc()).all()
        if form_foto.validate_on_submit():
            # if 'foto' in request.files:
                ...
                foto = Foto(imagem=nome_seguro, id_usuario=current_user.id, data_criacao=datetime.now())
...
def feed():
    ...
    for foto in fotos:
        foto.hora_formatada = foto.obter_hora_min_seg()
